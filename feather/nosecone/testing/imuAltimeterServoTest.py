...
def test_servo():
    print("Holding servo at 45° for 3 seconds...")
    my_servo.angle = 45
    time.sleep(3)

    print("Move servo to final position (45°) for 3 seconds...")
    my_servo.angle = 0
    time.sleep(3)
# ============================================================
...
# ---------------- IMU (ICM-20948) ----------------
log("Initializing ICM-20948 IMU...")
icm = None
for addr in (0x69, 0x68):
    try:
        icm = adafruit_icm20x.ICM20948(i2c, address=addr)
        log(f"  ICM-20948 found at 0x{addr:02X}")
        break
    except Exception as e:
        log(f"  Not at 0x{addr:02X}: {e}")

if icm is None:
    log("ERROR: ICM-20948 not detected. Check wiring.")

# ---------------- Altimeter (MPL3115A2) ----------------
log("Initializing MPL3115A2 altimeter...")
mpl = None
try:
    ...
    log(f"  MPL3115A2 not detected: {e}")

if icm is None and mpl is None:
    log("No sensors detected. Halting.")
    while True:
        time.sleep(1)

log("")
log("=" * 65)
log(f"{'ALT(m)':>8} {'P(hPa)':>8} {'T(C)':>6} | "
    f"{'Ax':>6} {'Ay':>6} {'Az':>6} | "
    f"{'Gx':>6} {'Gy':>6} {'Gz':>6} | "
    f"{'|a|':>6} {'|g|':>6}")
log("=" * 65)
